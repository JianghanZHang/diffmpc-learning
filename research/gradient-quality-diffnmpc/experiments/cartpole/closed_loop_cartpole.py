"""Closed-loop (50-step) gradient accuracy on the NONLINEAR cartpole — does the hard-box vs
slack-box outlier mechanism (found on the linear system) hold under nonlinear dynamics?

Regulation regime (small perturbations about UPRIGHT, theta~0) so the SQP converges in a few
iterations — the swing-up regime (theta~pi) needs ~300 SQP iters and is intractable for a
differentiable 50-step rollout grad. Control box ±umax still engages. Compares:
  A-hardbox  = TurboMPC hard box (use_slack=False)  -- the diffmpc default
  A-slack    = TurboMPC slack box (use_slack=True, gamma)
each AD (jax.grad through the rollout) vs convergence-checked FD of its own rollout.

    python research/gradient-quality-diffnmpc/experiments/cartpole/closed_loop_cartpole.py --smoke
    python research/gradient-quality-diffnmpc/experiments/cartpole/closed_loop_cartpole.py [--slack] --n_samples 6
"""
...
for _p in (os.path.join(_DL_ROOT, "src"), os.path.join(_DL_ROOT, "diffmpc2"), _HERE):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from benchmark_cartpole_coupling import build_cartpole_problem, NX, NU, Q_NOMINAL, R_NOMINAL  # noqa: E402
...
def _reg_x0(n, seed, scale=1.0):
    """Initial states near UPRIGHT (theta~0): regulation regime, few SQP iters needed."""
    rng = np.random.default_rng(seed)
    std = np.array([0.3, 0.3, 0.3, 0.3]) * scale
    return rng.normal(0.0, 1.0, size=(n, NX)) * std   # base = upright at origin (zeros)
...
def run(n_samples, seed, use_slack, sqp_iter, x0_scale=1.0, umax=UMAX):
    dyn, pp = build_cartpole_problem(horizon=HORIZON, umax=umax, dt=DT)
    pp = dict(pp)
    pp[QK] = Q_NOMINAL; pp[RK] = R_NOMINAL
    x0 = _reg_x0(n_samples, seed, x0_scale); x0_batch = jnp.asarray(x0)
    ...
    keep = ~flagged
    tag = "A-slack" if use_slack else "A-hardbox"
    print(f"\n=== {tag} (cartpole, sqp={sqp_iter}) vs FD, {int(keep.sum())}/{n_samples} non-flagged ===")
    if keep.any():
...
def main():
    ...
    p.add_argument("--umax", type=float, default=UMAX)
    p.add_argument("--smoke", action="store_true", help="tiny: n=2, sqp=5, sim_steps stays 50")
    args = p.parse_args()
    n, sqp = args.n_samples, args.sqp_iter
    if args.smoke:
        n, sqp = 2, 5
    print(f"Cartpole closed-loop (sim_steps={SIM_STEPS}, H={HORIZON}, umax={args.umax}, dt={DT}) "
          f"n={n} seed={args.seed} sqp={sqp} scale={args.x0_scale} slack={args.slack}")
    run(n, args.seed, args.slack, sqp, args.x0_scale, args.umax)
