...
bug). Run with the .venv-cudss python (matches external's FFI build):
    ...
    /home/jianghan/Workspace/diffmpc2/.venv-cudss/bin/python \
        research/gradient-quality-diffnmpc/experiments/linear_system/four_variant_benchmark.py --nseeds 10
"""
...
def main():
    ...
    ps = {(name, t): [] for name in CONFIGS for t in tols}   # per-sample cos arrays (one per seed)
    bs = {(name, t): [] for name in CONFIGS for t in tols}   # batch-summed cos_all (one scalar per seed)
    gt_bad = 0
    for seed in range(nseeds):
        ...
        keep = conv; gt_bad += int((~conv).sum())
        if a.smoke:
            gAD0 = ad(cf_hard, w, x0b)
            cs = np.array([_cos(gAD0[i], gFD[i]) for i in range(n)])[keep]
            print(f"  [sanity seed {seed}] cos(hardbox AD@{GT_TOL:.0e}, hardbox GT) "
                  f"med={np.median(cs):.4f} min={np.min(cs):.4f}  (should be ~1.0)", flush=True)
        for name in CONFIGS:
            for t in tols:
                cf = build_cost_fn(name, dyn, pp, w, x0b, A_sd, B_m, b_v, dt, t, SIM_STEPS)
                gAD = ad(cf, w, x0b)
                cos = np.array([_cos(gAD[i], gFD[i]) for i in range(n)])
                ps[(name, t)].append(cos[keep])
                bs[(name, t)].append(_cos(gAD[keep].sum(0), gFD[keep].sum(0)))
                jax.clear_caches(); gc.collect()
        print(f"  seed {seed} done ({time.time()-t0:.0f}s); GT non-converged so far={gt_bad}", flush=True)

    # ---- report ----
    print(f"\n=== cos(variant AD, common hard-box GT) | {nseeds} seeds, GT non-converged={gt_bad} ===")
    for t in tols:
        print(f"\n-- AD tol={t:.0e} --   {'variant':<20} | {'per-sample med(min)':>20} {'#<.99':>6} "
              f"{'#<0':>5} | {'batch-sum med(min)':>20}")
        for name in CONFIGS:
